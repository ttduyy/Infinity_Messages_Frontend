...
export class RequestBuilder {
  ...
  build<T = any>(options?: {
    ...
    const token = getCookie("JWT_TOKEN");  // Lấy JWT từ localStorage (hoặc một nguồn khác)
    if (token) {
      httpHeaders = httpHeaders.append('cookie', `${token}`);
    }

    const tokenSession = getCookie("JSESSIONID");  // Lấy JWT từ localStorage (hoặc một nguồn khác)
    if (tokenSession) {
      httpHeaders = httpHeaders.append('cookie', `${token}`);
    }
